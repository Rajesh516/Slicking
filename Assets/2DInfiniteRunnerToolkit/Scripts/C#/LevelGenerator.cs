...
public class LevelGenerator : MonoBehaviour 
{
	...
	void ScrollLevel()
	{
		...
		if (canModifySpeed)
			...
			scrollSpeed = defaultScroll + ((((maxScrollSpeedDist - (maxScrollSpeedDist - distance)) / maxScrollSpeedDist) * (maxScrollSpeed - defaultScroll))*0.60f);
	...
	{	
		...
		if (placeAtLoc == 1 || PlayerManager.Instance.IsIncreaseSpeedActive()) {
						go.GetComponent<ActivateChildOfGroupInFourthLayer> ().DeActivateChildren ();
						go.GetComponent<ActivateChildOfGroupInFourthLayer> ().ActivateInitialPlatform ();
				} else {
			go.GetComponent<ActivateChildOfGroupInFourthLayer> ().ActivateChildren ();
			go.GetComponent<ActivateChildOfGroupInFourthLayer> ().DeActivateInitialProblem();
				}
